...
class FormsTestCase(unittest.TestCase):
    ...
    def test_valid_create_form_with_charge(self):
        with self.app.test_request_context():
            ...
            form.charges.raw_data = [charge_form]

            form.extent.data = json.dumps(valid_geo_json)
            self.assertTrue(form.validate())

    def test_leasehold_is_true_and_leasehold_form_is_not_valid(self):
        with self.app.test_request_context():
            form = RegistrationForm()
            ...
            form.property_tenure.data = "Leasehold"
            form.property_class.data = "Absolute"
            form.price_paid.data = "10000.00"
            form.price_paid.raw_data = "10000.00"

            lease = LeaseholdForm()
            lease.lease_date.data = None
            lease.lease_term.data = None
            lease.lease_from.data = None
            form.leases.raw_data = [lease]
